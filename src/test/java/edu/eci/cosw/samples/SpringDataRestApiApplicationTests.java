...
//| topPatients(N)| Registrar 3 pacientes. Uno sin consultas, otro con una, y el último con dos consultas. Probar usando N=1  y esperar una lista con los dos pacientes correspondientes.| centered      | 
	...
	public void contextLoads() {
        ...
        public void patientLoadTest1(){
            Paciente p = new Paciente(new PacienteId(1030675544, "cc"), "Cristian Pinzon", new Date(2018));
            pr.save(p);
            try {
                assertEquals("Ambos usuarios son iguales", p.getId().getId(), services.getPatient(1030675544, "cc").getId().getId());
        ...
        public void topPatientsLoadTest1(){
            ...
            Paciente p = new Paciente(new PacienteId(1030675544, "cc"), "Cristopher Test", new Date(2018),con);
            pr.save(p);
            try {
        ...
        public void topPatientsLoadTest2(){
            Paciente p1 = new Paciente(new PacienteId(1030675544, "cc"), "Cristian Test", new Date(2018));
            pr.save(p1);
            
            Consulta c1 =new Consulta(new Date(2018), "estoy en excelente forma");
            Set<Consulta> con1=new HashSet<>();con1.add(c1);
            Paciente p2 = new Paciente(new PacienteId(1030675555, "cc"), "Camilo Test", new Date(2018),con1);
            pr.save(p2);
            ...
            Paciente p3 = new Paciente(new PacienteId(10306755, "cc"), "Hernandez Test", new Date(2018),con2);
            pr.save(p3);
            
            try {
                assertEquals("Existen solo 3 pacientes con mas de 1 consultas", 3, services.topPatients(1).size());
            } catch (ServicesException ex) {
                Logger.getLogger(SpringDataRestApiApplicationTests.class.getName()).log(Level.SEVERE, null, ex);
            }
        }
